      subroutine teos(temp,dens,ein,pres,cs,det,dpt,table)
      include 'implno.dek'
      include 'vector_eos.dek'
      
      double precision temp,dens,ein,pres,cs,det,dpt,table(577013)
!      integer N
...
! hydrogen, heliu, and carbon
      ...
      xmass(2) = 0.0d0 ; aion(2)  = 4.0d0  ; zion(2)  = 2.0d0
      xmass(3) = 1.00d0 ; aion(3)  = 12.0d0 ; zion(3)  = 6.0d0
...
! routine helmeos_orig as helmeos with the table read inside the routine
      ...
      subroutine read_helm_table_0(table)
      include 'implno.dek'
...
!      write(6,*)
	...
	do i=1,imax
	  ...
	  is=is+1
       ...
       do j=1,jmax-1
        table(is)=dt_sav(j)
        table(is+1)=dt2_sav(j)
        table(is+2)=dti_sav(j)
        table(is+3)=dt2i_sav(j)
        table(is+4)=dt3i_sav(j)
  	is=is+5
       end do
       do i=1,imax-1
        table(is)=dd_sav(i)  
        table(is+1)=dd2_sav(i)  
        table(is+2)=ddi_sav(i)  
        table(is+3)=dd2i_sav(i) 
        table(is+4)=dd3i_sav(i)
        is=is+5
       enddo       
	...
	do j=1,jmax
          do i=1,imax
             table(is)=f(i,j)
	     table(is+1*imax*jmax)=fd(i,j)
	     table(is+2*imax*jmax)=ft(i,j)
	     table(is+3*imax*jmax)=fdd(i,j)
	     table(is+4*imax*jmax)=ftt(i,j)
	     table(is+5*imax*jmax)=fdt(i,j)
             table(is+6*imax*jmax)=fddt(i,j)
	     table(is+7*imax*jmax)=fdtt(i,j)
	     table(is+8*imax*jmax)=fddtt(i,j)
	     table(is+9*imax*jmax)=dpdf(i,j)
             table(is+10*imax*jmax)=dpdfd(i,j)
	     table(is+11*imax*jmax)=dpdft(i,j)
	     table(is+12*imax*jmax)=dpdfdt(i,j)
             table(is+13*imax*jmax)=ef(i,j)
             table(is+14*imax*jmax)=efd(i,j)
             table(is+15*imax*jmax)=eft(i,j)
	     table(is+16*imax*jmax)=efdt(i,j)
             table(is+17*imax*jmax)=xf(i,j)
             table(is+18*imax*jmax)=xfd(i,j)
             table(is+19*imax*jmax)=xft(i,j)
             table(is+20*imax*jmax)=xfdt(i,j)
             is=is+1
        enddo
       enddo	
...
!    d_iat=d(iat)
    ...
    dti_sav(jat)=table(is+jat+1)
    dt2i_sav(jat)=table(is+jat+2)
    dt3i_sav(jat)=table(is+jat+3)
  	is=is+5*(jmax-1)
 
    dd_sav(iat)=table(is+iat-1)  
    dd2_sav(iat)=table(is+iat)
    ddi_sav(iat)=table(is+iat+1) 
    dd2i_sav(iat)=table(is+iat+2) 
    dd3i_sav(iat)=table(is+iat+3)
    is=is+5*(imax-1)
...
! access the table locations only once
        fi(1)  = table(is+iat+(jat-1)*imax);
        fi(2)  = table(is+iat+1+(jat-1)*imax);
        fi(3)  = table(is+iat+jat*imax)
        fi(4)  = table(is+iat+1+jat*imax)
        fi(5)  = table(is+2*imax*jmax+iat+(jat-1)*imax)
        fi(6)  = table(is+2*imax*jmax+iat+1+(jat-1)*imax)
        fi(7)  = table(is+2*imax*jmax+iat+jat*imax)
        fi(8)  = table(is+2*imax*jmax+iat+1+jat*imax)
        fi(9)  = table(is+4*imax*jmax+iat+(jat-1)*imax)
        fi(10)  = table(is+4*imax*jmax+iat+1+(jat-1)*imax)
        fi(11)  = table(is+4*imax*jmax+iat+jat*imax)
        fi(12)  = table(is+4*imax*jmax+iat+1+jat*imax)
        fi(13) = table(is+imax*jmax+iat+(jat-1)*imax)
        fi(14) = table(is+imax*jmax+iat+1+(jat-1)*imax)
        fi(15) = table(is+imax*jmax+iat+jat*imax)
        fi(16) = table(is+imax*jmax+iat+1+jat*imax)
        fi(17) = table(is+3*imax*jmax+iat+(jat-1)*imax)
        fi(18) = table(is+3*imax*jmax+iat+1+(jat-1)*imax)
        fi(19) = table(is+3*imax*jmax+iat+jat*imax)
        fi(20) = table(is+3*imax*jmax+iat+1+jat*imax)
        fi(21) = table(is+5*imax*jmax+iat+(jat-1)*imax)
        fi(22) = table(is+5*imax*jmax+iat+1+(jat-1)*imax)
        fi(23) = table(is+5*imax*jmax+iat+jat*imax)
        fi(24) = table(is+5*imax*jmax+iat+1+jat*imax)
        fi(25) = table(is+6*imax*jmax+iat+(jat-1)*imax)
        fi(26) = table(is+6*imax*jmax+iat+1+(jat-1)*imax)
        fi(27) = table(is+6*imax*jmax+iat+jat*imax)
        fi(28) = table(is+6*imax*jmax+iat+1+jat*imax)
        fi(29) = table(is+7*imax*jmax+iat+(jat-1)*imax)
        fi(30) = table(is+7*imax*jmax+iat+1+(jat-1)*imax)
        fi(31) = table(is+7*imax*jmax+iat+jat*imax)
        fi(32) = table(is+7*imax*jmax+iat+1+jat*imax)
        fi(33) = table(is+8*imax*jmax+iat+(jat-1)*imax)
        fi(34) = table(is+8*imax*jmax+iat+1+(jat-1)*imax)
        fi(35) = table(is+8*imax*jmax+iat+jat*imax)
        fi(36) = table(is+8*imax*jmax+iat+1+jat*imax)
...
! look in the pressure derivative only once
        ...
        fi(9) = table(is+imax*jmax+iat+(jat-1)*imax)
        fi(10) = table(is+imax*jmax+iat+1+(jat-1)*imax)
        fi(11) = table(is+imax*jmax+iat+jat*imax)
        fi(12) = table(is+imax*jmax+iat+1+jat*imax)
        fi(13) = table(is+3*imax*jmax+iat+(jat-1)*imax)
        fi(14) = table(is+3*imax*jmax+iat+1+(jat-1)*imax)
        fi(15) = table(is+3*imax*jmax+iat+jat*imax)
        fi(16) = table(is+3*imax*jmax+iat+1+jat*imax)
